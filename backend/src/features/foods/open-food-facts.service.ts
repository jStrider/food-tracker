...
export class OpenFoodFactsService {
  ...
  async searchByName(query: string): Promise<FoodSearchResultDto[]> {
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.get(`${this.baseUrl}/cgi/search.pl`, {
          params: {
          ...
          catchError((error) => {
            this.logger.error(`OpenFoodFacts API error for query "${query}":`, error.message);
      ...
      const results = response.data.products
        ...
        .filter((food: FoodSearchResultDto) => food.name && food.name !== 'Unknown Product')
        .sort((a: FoodSearchResultDto, b: FoodSearchResultDto) => (b.confidence || 0) - (a.confidence || 0));

      this.logger.log(`Found ${results.length} products for query: ${query}`);
      return results;
    } catch (error) {
      this.logger.error(`Failed to search OpenFoodFacts for "${query}":`, error.message);
  ...
  async searchByBarcode(barcode: string): Promise<FoodSearchResultDto | null> {
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.get(`${this.baseUrl}/product/${sanitizedBarcode}.json`, {
          params: {
            fields: 'code,product_name,brands,nutriments,image_url,serving_size',
          },
          timeout: this.REQUEST_TIMEOUT,
          ...
          retry(this.MAX_RETRIES),
          catchError((error) => {
            this.logger.error(`OpenFoodFacts API error for barcode "${barcode}":`, error.message);
            return throwError(() => error);
          })
        )
      );
      ...
      if (response.data.status === 1 && response.data.product) {
        const result = this.mapOpenFoodFactsProduct(response.data.product, sanitizedBarcode);
      ...
      this.logger.error(`Failed to fetch product by barcode "${barcode}":`, error.message);
      throw new Error(`Barcode search failed: ${error.message}`);
    }
  }

  private mapOpenFoodFactsProduct(product: any, searchTerm?: string): FoodSearchResultDto {
    const nutriments = product.nutriments || {};
    
    // Extract and clean product name
    const productName = this.cleanProductName(product.product_name || product.generic_name || 'Unknown Product');
    ...
    const confidence = searchTerm ? this.calculateConfidence(productName, brand, searchTerm) : 1.0;
  ...
  private normalizeNutrition(nutriments: any): any {
    ...
    return {
      calories: getValue('energy-kcal_100g') || getValue('energy_100g') / 4.184 || 0,
  ...
  private calculateConfidence(productName: string, brand: string, searchTerm: string): number {
