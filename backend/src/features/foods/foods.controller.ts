import { Controller, Get, Post, Put, Delete, Body, Param, Query, BadRequestException, UsePipes, ValidationPipe } from '@nestjs/common';
import { FoodsService } from './foods.service';
import { FoodCacheService, CacheStats } from './food-cache.service';
import { FoodsHealthService } from './foods.health';
import { CreateFoodDto, UpdateFoodDto, CreateFoodEntryDto, UpdateFoodEntryDto, SearchFoodDto } from './dto';
...
export class FoodsController {
  ...
  searchFoods(@Query('q') query?: string, @Query('barcode') barcode?: string) {
    if (!query && !barcode) {
      throw new BadRequestException('Either query (q) or barcode parameter is required');
    }
    
    if (barcode) {
      if (barcode.length < 8 || barcode.length > 13) {
        throw new BadRequestException('Barcode must be between 8 and 13 digits');
      }
  ...
  @Post(':mealId/entries')
  addFoodToMeal(@Param('mealId') mealId: string, @Body() createFoodEntryDto: CreateFoodEntryDto) {
    return this.foodsService.addFoodToMeal(mealId, createFoodEntryDto);
  }

  @Put('entries/:entryId')
  updateFoodEntry(@Param('entryId') entryId: string, @Body() updateFoodEntryDto: UpdateFoodEntryDto) {
