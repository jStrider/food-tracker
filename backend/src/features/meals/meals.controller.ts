...
export class MealsController {
  ...
  @ApiQuery({
    name: "includeFoods",
    required: false,
    description: "Include food entries",
  })
  async findAll(
    @Query(ValidationPipe) query: MealQueryDto,
  ...
  async getStats(
    @Query(ValidationPipe) query: MealStatsQueryDto,
  ): Promise<MealStats> {
    this.logger.log(
      `Getting meal stats with filters: ${JSON.stringify(query)}`,
    );
  ...
  async calculateNutritionProgress(
    ...
    return this.nutritionIntegrationService.calculateNutritionProgress(
      date,
      goals,
    );
  }
  ...
  @ApiParam({ name: "date", description: "Date in YYYY-MM-DD format" })
  async getDailyMacroDistribution(
    @Param("date") date: string,
  ...
  @ApiParam({ name: "id", description: "Meal UUID" })
  async getMealMacroDistribution(
    @Param("id", ParseUUIDPipe) id: string,
  ): Promise<MacroDistribution> {
  ...
  async getNutritionTrends(
    ...
    return this.nutritionIntegrationService.calculateNutritionTrends(
      startDate,
      endDate,
    );
  }
