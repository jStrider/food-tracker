...
export class CalendarController {
  ...
  @ApiResponse({ status: 200, description: 'Monthly calendar data with goal progress' })
  getMonthViewWithGoals(
    @Query('month') month: string,
    @Query('year') year: string,
    @Body() goals: NutritionGoals,
  ) {
    return this.calendarService.getMonthView(parseInt(month), parseInt(year), goals);
  }
  ...
  @ApiResponse({ status: 200, description: 'Weekly calendar data with goal progress' })
  ...
  @Get('day')
  @ApiOperation({ summary: 'Get detailed day view with all meals and nutrition' })
