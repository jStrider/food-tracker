...
describe('NutritionService', () => {
  ...
  const createMockMealWithNutrition = (meal: any, foodEntries: any[]) => {
    return {
      ...
      foods: foodEntries.map(entry => ({
        ...entry,
        calculatedCalories: entry.calories || (entry.food.calories * entry.quantity) / 100,
        calculatedProtein: entry.protein || (entry.food.protein * entry.quantity) / 100,
        calculatedCarbs: entry.carbs || (entry.food.carbs * entry.quantity) / 100,
  ...
  describe('getMealNutrition', () => {
    it('should calculate nutrition for a single meal', async () => {
      const mockMeal = createMockMealWithNutrition(
    ...
    it('should throw error when meal not found', async () => {
      ...
      await expect(service.getMealNutrition('999')).rejects.toThrow('Meal not found');
    });
  });
    ...
    it('should calculate daily nutrition summary', async () => {
      ...
      const mockMeals = [
        ...
        createMockMealWithNutrition(
          ...
          [
            { ...fixtures.foodEntries.lunchChicken, food: fixtures.foods.chickenBreast },
  ...
  describe('getMonthlyNutrition', () => {
    it('should get nutrition for each day of the month', async () => {
      ...
      jest.spyOn(service, 'getDailyNutrition').mockResolvedValue(mockDailyNutrition);
  ...
  describe('getMacroBreakdown', () => {
    it('should calculate macro breakdown percentages', () => {
      ...
      expect(result.protein + result.carbs + result.fat).toBeLessThanOrEqual(100);
    });
  ...
  describe('Private Methods', () => {
    it('should calculate nutrition from food entries correctly', () => {
      ...
      const result = (service as any).calculateNutritionFromFoodEntries(foodEntries);
    ...
    it('should aggregate nutrition correctly', () => {
      const summaries = [
        { calories: 100, protein: 10, carbs: 20, fat: 5, fiber: 2, sugar: 8, sodium: 100 },
        { calories: 200, protein: 20, carbs: 30, fat: 10, fiber: 3, sugar: 12, sodium: 200 },
      ];
    ...
    it.each([
      ...
      [110, 'met'],
      [115, 'over'],
    ])('should return %s status for %d%% of goal', (percentage, expectedStatus) => {
