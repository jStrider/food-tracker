...
import { MealsService, MealFilterOptions } from '../features/meals/meals.service';
import { FoodsService } from '../features/foods/foods.service';
import { NutritionService, NutritionGoals } from '../features/nutrition/nutrition.service';
...
export class McpService {
  ...
  getAvailableTools() {
    return {
      tools: [
        ...
        {
          name: 'get_meals',
          description: 'Get meals with optional filtering by date, date range, or category',
          inputSchema: {
            type: 'object',
            properties: {
              date: { type: 'string', description: 'Specific date in YYYY-MM-DD format' },
              startDate: { type: 'string', description: 'Start date for range in YYYY-MM-DD format' },
        ...
        {
          name: 'create_meal',
          description: 'Create a new meal with auto-categorization if category not provided',
          inputSchema: {
            type: 'object',
            properties: {
              name: { type: 'string' },
              ...
              time: { type: 'string', description: 'Time in HH:MM format (24-hour). If provided, will be used for auto-categorization' },
            },
            required: ['name', 'date'],
          },
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              category: { type: 'string', enum: ['breakfast', 'lunch', 'dinner', 'snack'] },
              date: { type: 'string' },
              time: { type: 'string', description: 'Time in HH:MM format (24-hour). If provided, will be used for auto-categorization if category not specified' },
            },
            required: ['mealId'],
          },
        },
        ...
        {
          name: 'search_foods',
          description: 'Search for foods by name or barcode from local cache and OpenFoodFacts',
          inputSchema: {
            type: 'object',
            properties: {
        ...
        },
        {
          ...
          description: 'Get macronutrient breakdown percentages for nutrition data',
          inputSchema: {
            type: 'object',
            properties: {
              calories: { type: 'number' },
        ...
        // Utility operations
        {
          ...
          description: 'Get current meal categorization time ranges or test custom ranges',
          inputSchema: {
            type: 'object',
            properties: {
              customRanges: {
  ...
  async callTool(toolName: string, params: any) {
    try {
      switch (toolName) {
        ...
        case 'search_foods':
          if (params.barcode) {
            const food = await this.foodsService.searchByBarcode(params.barcode);
            return food ? [food] : [];
          }
        ...
        case 'get_monthly_nutrition':
          return this.nutritionService.getMonthlyNutrition(params.month, params.year);
        
        case 'compare_to_goals':
          return this.nutritionService.compareToGoals(params.date, params.goals);
  ...
  async getResource(resourceUri: string) {
    try {
      ...
      switch (resource) {
        ...
        case 'nutrition':
          ...
          if (subResource === 'daily') {
            ...
            const now = new Date();
            return this.nutritionService.getMonthlyNutrition(now.getMonth() + 1, now.getFullYear());
          }
        ...
        case 'system':
          if (subResource === 'info') {
            return {
              ...
              version: '1.0.0',
              description: 'Complete nutrition tracking system with OpenFoodFacts integration',
              capabilities: {
      ...
      throw new Error(`Error fetching resource ${resourceUri}: ${error.message}`);
    }
  }
}