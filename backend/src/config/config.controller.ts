...
export class ConfigController {
  ...
  @ApiOperation({
    ...
    description:
      "Returns non-sensitive configuration information for debugging and monitoring",
  })
  @ApiResponse({
    status: 200,
      ...
      properties: {
        ...
        externalApis: {
          ...
          properties: {
            openFoodFacts: {
              type: "string",
              example: "https://world.openfoodfacts.org/api/v0",
            },
          },
        },
        throttle: {
  ...
  validateConfiguration() {
    ...
    if (this.configService.isProduction()) {
      ...
      if (this.configService.getDatabasePath().includes("data/")) {
        warnings.push(
          "Using local database in production (consider external database)",
        );
      }
    }
